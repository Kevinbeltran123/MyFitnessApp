...
void main() {
  ...
  test(
    'routine data persists with exercises and schedule after restart',
    () async {
      ...
      final RoutineRepositoryIsar repoReopened = RoutineRepositoryIsar(
        reopened,
      );
  ...
  test(
    ...
    () async {
      ...
      final BodyMetric metricTwo = MockDataGenerator.bodyMetric(
        ...
        bodyFat: 19.8,
        muscleMass: 35.2,
        measurements: const <String, double>{'cintura': 87.5, 'pecho': 101.2},
      );
      ...
      final MetricsRepositoryIsar repoReopened = MetricsRepositoryIsar(
        reopened,
      );

      final List<BodyMetric> persistedMetrics = await repoReopened
          .watchMetrics()
          .first;
      expect(persistedMetrics.length, 2);
      expect(
        persistedMetrics.first.id,
        'metric-2',
      ); // Sorted desc by recordedAt
      expect(persistedMetrics.last.measurements['cintura'], 88.3);

      final MetabolicProfile? persistedProfile = await repoReopened
  ...
  test('workout sessions persist with detailed logs after restart', () async {
    ...
    final List<RoutineSession> sessions = await repoReopened.getAllSessions();
  ...
  test(
    ...
    () async {
      ...
      final List<Routine> active = await repoReopened
          .watchAll(includeArchived: false)
          .first;
      expect(active, isEmpty);

      final List<Routine> archived = await repoReopened
          .watchAll(includeArchived: true)
          .first;
      expect(
        archived.map((Routine r) => r.id).toSet(),
      ...
      final List<Routine> restoredList = await repoReopened
          .watchAll(includeArchived: false)
          .first;
